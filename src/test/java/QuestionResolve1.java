...
public class QuestionResolve1 {

    public static final Pattern themePattern = Pattern.compile("\\s*\\{(?<cat>.*)}\\s*");
    public static final Pattern selectOptionPattern = Pattern.compile("^[\\t\\s\\r\\n]*(?<optKey>[A-Za-z]+)\\s*(\\.|、|．|\\t)*\\s*(?<optVal>.*)[\\t\\s\\r\\n]*$");
    public static final Pattern questionPattern = Pattern.compile("^\\d+\\s*(\\.|、|．)\\s*(?<question>.*)$");
    public static final Pattern selectAnswerPattern = Pattern.compile("^答案\\s*(:|：)\\s*(?<answer>.*)[\\t\\s\\r\\n]*$");
    public static final Pattern explainPattern = Pattern.compile("^答案解析\\s*(:|：)\\s*(?<explain>.*)[\\t\\s\\r\\n]*$");

    public static List<Question> getQuestions() {
        List<String> strings = FileUtil.readLines(new File("D:\\workspace\\idea\\spring-boot-online-exam\\doc\\大企业税收管理.txt"), CharsetUtil.UTF_8);
